    ...
    <div class='content-box'>
      ...
      <v-form v-model='valid'>
        <v-text-field
          ...
          @blur='searchCEP'
        ></v-text-field>
        <v-text-field v-model='state' :rules='stateRules' label='Estado' disabled />
        <v-text-field v-model='city' :rules='cityRules' label='Cidade' disabled />
        <v-text-field v-model='address' :rules='addressRules' label='Rua' required />
        <v-text-field v-model='neighborhood' :rules='neighborhoodRules' label='Bairro' required />
        <v-text-field v-model='number' :rules='numberRules' label='Nº' required />
        <v-text-field v-model='complement' label='Complemento' />
        <v-btn class='mr-4 btn mt-10 color-white' type='submit' :disabled='!valid' @click='goForward'>
          Avançar
        </v-btn>
      </v-form>
    </div>
...
export default {
  ...
  mounted() {
    ...
    this.neighborhood = this.$store.state.signupStore.userRegister.neighborhood || ''
    this.number = this.$store.state.signupStore.userRegister.number || ''
    this.complement = this.$store.state.signupStore.userRegister.complement || ''
  },
  methods: {
    clearFormCEP() {
      this.state = ''
