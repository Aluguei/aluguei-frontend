  ...
  <div class='d-flex align-center justify-center'>
    <div class='content-box'>
      ...
      <v-form v-model='valid'>
        <v-text-field v-model='phoneOne' v-mask="'(##) # ####-####'" :rules='phoneOneRules' label='Telefone primário'
                      required />
        <v-text-field v-show='phoneOne' v-model='phoneTwo' v-mask="'(##) # ####-####'"
                      label='Telefone secundário (opcional)' />
        <v-btn class='mr-4 btn mt-10 color-white' color='success' type='submit' :loading='loading'
...
export default {
  ...
  data: () => ({
    ...
    phoneTwo: null,
    phoneOneRules: [
      (v) => !!v || 'Telefone primário é obrigatório'
    ]
  }),
  mounted() {
  ...
  methods: {
    registerUser() {
      ...
      this.loading = true
      this.valid = true
      this.$axios.$post('/api/auth/register', this.$store.state.signupStore.userRegister).then(response => {
        ...
        this.loading = false
        this.snackbar = true
        this.text = 'Erro ao tentar se registar, por favor tente novamente. Se o erro persistir entre em contato com nossa equipe de suporte.'
      })
    },
    goBack() {
